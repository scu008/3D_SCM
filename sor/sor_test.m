...
%         Wt = beamformer(ant_num(1,:), rx_angle(1:2,:,:,:), N_s, 1, m_path);
        [Wt, Wd, sum_rate] = sor_beamformer(ant_num,N_rf, N_d, H, fft_len, snr(j));
...
%         [Dsym, ~, bd_H, Wd] = BD_precoding(Dsym, He, A);
        ...
        C_simulation(j) = C_simulation(j) + sum_rate / iter;
    end
...
figure(2)
plot(snr, C_simulation,'-o'), hold on
movegui('northeast');
